...
@interface ESHouseMapViewController ()

@end

@implementation ESHouseMapViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _esMapAFNetWorkManager = [[AFNetWorkManager alloc] init];
        _esMapAFNetWorkManager.delegate = self;
    }
    return self;
}
-(void)showBackGroundForTableView{
    ...
    backgroundForTableView.frame=CGRectMake(0.0, 100.0, self.mapView.bounds.size.width, self.mapView.bounds.size.height-100.0);
    [UIView commitAnimations];
}
-(void)hiddenBackGroundForTableView{
    [UIView beginAnimations:nil context:nil];
    [UIView setAnimationDuration:.5];
    backgroundForTableView.frame=CGRectMake(0.0, self.mapView.bounds.size.height, self.mapView.bounds.size.width, self.mapView.bounds.size.height-100.0);
    [UIView commitAnimations];
...
-(void)addBackGroudForTableView{
    ...
    eshMapAreaList.eshsArea_commandcode=119;
    eshMapAreaList.eshsArea_page=1;
}
...
-(void)getHouseSourceNumber{
    ...
    NSLog(@"location:%f,%f",maxMapCoordinate.latitude,maxMapCoordinate.longitude);
    esmHouseSource=[[esMapHouseSource alloc] init];
    esmHouseSource.es_commandcode=118;
    ...
    [self.esMapAFNetWorkManager getESMapHouseSource:esmHouseSource];
}
    ...
    if (isSuccess) {
        ...
        if (code==118) {
            ...
            if (formSearchVCGoBack) {//是从搜索小区界面返回的
                ...
                NSString *houseCount=[NSString stringWithFormat:@"%d",_es_searchAreaCount];
...
- (void)mapView:(BMKMapView *)mapView regionDidChangeAnimated:(BOOL)animated{
    ...
    /*
     {"commandcode":118,"REQUEST_BODY":{"city":"昆明","minlat":24.585632,"maxlat":25.200005,"minlng":102.622851,"maxlng":103.002301,"zoomLevel":12.000000}}
     */
}
...
//version2.6 didAddAnnotationViews 调用多次，version2.5 didAddAnnotationViews 调用1次，
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *hx=[NSString stringWithFormat:@"%@  %@平米",[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"housetype"],[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"area"]];
    cell.hxString=hx;
    cell.moneyString=[NSString stringWithFormat:@"%@",[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"totalprice"]];
    return cell;
}
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    
    if (indexPath.row==self.es_mapAreaList.count-1&&esh_mapAreaNextPage&&!self.esMapAFNetWorkManager.isLoading){//列表是二手房源列表&&显示最后一行&&当前没有请求在进行&&二手房源有下一页
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    esHouseDetailInfoVC.titleNameStr=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"community"];
    esHouseDetailInfoVC.hidesBottomBarWhenPushed = YES;
    esHouseDetailInfoVC.esnid=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"nid"];
    esHouseDetailInfoVC.eshouseArea=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"area"];
    esHouseDetailInfoVC.escamera=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"camera"];
    esHouseDetailInfoVC.escid=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"cid"];
    esHouseDetailInfoVC.escommunity=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"title"];
    esHouseDetailInfoVC.eshouseType=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"housetype"];
    esHouseDetailInfoVC.estotalPrice=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"totalprice"];
    esHouseDetailInfoVC.iconurl=[[self.es_mapAreaList objectAtIndex:indexPath.row] objectForKey:@"iconurl"];
...
- (BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar{
    ...
    searchVC.commandcode=ESHouseSearchArea_CommandCode;
    NSUserDefaults *userDefault=[NSUserDefaults standardUserDefaults];
